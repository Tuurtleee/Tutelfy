...
function Footer({token, setCurrentView, setCurrentPlaylist}){
    ...
    return (
        <div className='Footer-container'>
          <div className="last-played">
            {lastplayed!=null  && <div className="flex"><img src={lastplayed.album.images[0].url} alt="" className='nowplaying' onClick={()=>{
          fetch(lastplayed.album.href, {
              method: 'GET',
              headers: {
                'Authorization':'Bearer '+token,
                'Content-Type': 'application/json'
            }}).then((response)=>{return response.json()}).then((json)=>{
              setCurrentPlaylist(json)
              setCurrentView("playlist");
            })
          }}/><div className='prod-info played'>
              <span className='song'>{lastplayed.name}</span><span className='artist'>{lastplayed.artists[0].name}</span>
              </div></div>}
            </div>
            ...
            <div className="progress-bar-container">
                <input type="range" min="0" max={lastplayed!=null?lastplayed.duration_ms/1000:10}/>
            </div>
        </div>
    )
